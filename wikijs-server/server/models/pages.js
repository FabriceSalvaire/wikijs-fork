...
export default class Page extends Model {
    ...
    static async createPage(opts) {
        ...
        // -> Check for duplicate
        const dupCheck = await WIKI.models.pages.query().select('id').where('localeCode', opts.locale).where(
            'path',
            opts.path
        ).first()
        if (dupCheck)
        ...
        if (shouldConvert) {
            ...
            if (sourceContentType === 'markdown' && targetContentType === 'html') {
                ...
                if ($.root().children().length > 0) {
                    ...
                    convertedContent = $.html('body').replace('<body>', '').replace('</body>', '').replace(
                        /&#x([0-9a-f]{1,6});/ig,
                        (entity, code) => {
    ...
    static async movePage(opts) {
        ...
        // -> Validate path
        if (
            opts.destinationPath.includes('.') || opts.destinationPath.includes(' ') ||
            opts.destinationPath.includes('\\') || opts.destinationPath.includes('//')
        ) {
            throw new WIKI.Error.PageIllegalPath()
        }
    ...
    static async migrateToLocale({ sourceLocale, targetLocale }) {
        return WIKI.models.pages.query()
            ...
            .whereNotExists(function () {
                this.select('id').from('pages AS pagesm').where('pagesm.localeCode', targetLocale).andWhereRaw(
