...
export default class User extends Model {
    ...
    verifyTFA(code) {
        ...
        return (result && lodash.has(result, 'delta') && result.delta === 0)
    }

    getGlobalPermissions() {
        return lodash.uniq(lodash.flatten(lodash.map(this.groups, 'permissions')))
    }

    getGroups() {
        return lodash.uniq(lodash.map(this.groups, 'id'))
    }
    ...
    static async processProfile({ profile, providerKey }) {
        const provider = lodash.get(WIKI.auth.strategies, providerKey, {})
        provider.info = lodash.find(WIKI.data.authentication, ['key', provider.stategyKey])

        // Find existing user
        let user = await WIKI.models.users.query().findOne({
            providerId: lodash.toString(profile.id),
            providerKey
        })

        // Parse email
        let primaryEmail = ''
        if (lodash.isArray(profile.emails)) {
            const e = lodash.find(profile.emails, ['primary', true])
            primaryEmail = e ? e.value : lodash.first(profile.emails).value
        } else if (lodash.isArray(profile.email))
            primaryEmail = lodash.first(lodash.flattenDeep([profile.email]))
        else if (lodash.isString(profile.email) && profile.email.length > 5)
            primaryEmail = profile.email
        else if (lodash.isString(profile.mail) && profile.mail.length > 5)
            primaryEmail = profile.mail
        else if (profile.user && profile.user.email && profile.user.email.length > 5)
            primaryEmail = profile.user.email
        else
            throw new Error('Missing or invalid email address from profile.')
        primaryEmail = lodash.toLower(primaryEmail)
        ...
        // Parse display name
        let displayName = ''
        if (lodash.isString(profile.displayName) && profile.displayName.length > 0)
            displayName = profile.displayName
        else if (lodash.isString(profile.name) && profile.name.length > 0)
        ...
        if (profile.picture && Buffer.isBuffer(profile.picture))
            ...
            pictureUrl = lodash.truncate(lodash.get(profile, 'picture', lodash.get(user, 'pictureUrl', null)), {
                length: 255,
                omission: ''
            })
        }
        ...
        if (provider.selfRegistration) {
            ...
            if (lodash.get(provider, 'domainWhitelist', []).length > 0) {
                const emailDomain = lodash.last(primaryEmail.split('@'))
                if (!lodash.includes(provider.domainWhitelist, emailDomain))
    ...
    static async login(opts, context) {
        if (lodash.has(WIKI.auth.strategies, opts.strategy)) {
            const selStrategy = lodash.get(WIKI.auth.strategies, opts.strategy)
            if (!selStrategy.isEnabled)
                throw new WIKI.Error.AuthProviderInvalid()

            const strInfo = lodash.find(WIKI.data.authentication, ['key', selStrategy.strategyKey])

            // Inject form user/pass
            if (strInfo.useForm) {
                lodash.set(context.req, 'body.email', opts.username)
                lodash.set(context.req, 'body.password', opts.password)
                lodash.set(context.req.params, 'strategy', opts.strategy)
            }
    ...
    static async afterLoginChecks(
        ...
        if (user.groups && user.groups.length > 0) {
            for (const grp of user.groups) {
                if (!lodash.isEmpty(grp.redirectOnLogin) && grp.redirectOnLogin !== '/') {
    ...
    static async refreshToken(user) {
        if (lodash.isSafeInteger(user)) {
            user = await WIKI.models.users.query().findById(user).withGraphFetched('groups').modifyGraph(
                'groups',
                (builder) => {
        ...
        } else if (lodash.isNil(user.groups)) {
    ...
    static async loginForgotPassword({ email }, context) {
        ...
        await WIKI.mail.send({
            ...
            text:
                `A password reset was requested for wiki ${WIKI.config.title}. Open the following link to proceed: ${WIKI.config.host}/login-reset/${resetToken}`
        })
    }
    ...
    static async createNewUser(
        { providerKey, email, passwordRaw, name, groups, mustChangePassword, sendWelcomeEmail }
    ) {
        // Input sanitization
    ...
    static async updateUser(
        { id, email, name, newPassword, groups, location, jobTitle, timezone, dateFormat, appearance }
    ) {
        const usr = await WIKI.models.users.query().findById(id)
        if (usr) {
            let usrData = {}
            if (!lodash.isEmpty(email) && email !== usr.email) {
                ...
                if (dupUsr)
                    throw new WIKI.Error.AuthAccountAlreadyExists()
                usrData.email = lodash.toLower(email)
            }
            if (!lodash.isEmpty(name) && name !== usr.name)
                usrData.name = lodash.trim(name)
            if (!lodash.isEmpty(newPassword)) {
                if (newPassword.length < 6)
                    throw new WIKI.Error.InputInvalid('Password must be at least 6 characters!')
                usrData.password = newPassword
            }
            if (lodash.isArray(groups)) {
                const usrGroupsRaw = await usr.$relatedQuery('groups')
                const usrGroups = lodash.map(usrGroupsRaw, 'id')
                // Relate added groups
                const addUsrGroups = lodash.difference(groups, usrGroups)
                for (const grp of addUsrGroups)
                    await usr.$relatedQuery('groups').relate(grp)
                // Unrelate removed groups
                const remUsrGroups = lodash.difference(usrGroups, groups)
                for (const grp of remUsrGroups)
                    await usr.$relatedQuery('groups').unrelate().where('groupId', grp)
            }
            if (!lodash.isEmpty(location) && location !== usr.location)
                usrData.location = lodash.trim(location)
            if (!lodash.isEmpty(jobTitle) && jobTitle !== usr.jobTitle)
                usrData.jobTitle = lodash.trim(jobTitle)
            if (!lodash.isEmpty(timezone) && timezone !== usr.timezone)
                usrData.timezone = timezone
            if (!lodash.isNil(dateFormat) && dateFormat !== usr.dateFormat)
                usrData.dateFormat = dateFormat
            if (!lodash.isNil(appearance) && appearance !== usr.appearance)
    ...
    static async register({ email, password, name, verify = false, bypassChecks = false }, context) {
        ...
        if (localStrg.selfRegistration || bypassChecks) {
            ...
            // Check if email domain is whitelisted
            if (lodash.get(localStrg, 'domainWhitelist.v', []).length > 0 && !bypassChecks) {
                const emailDomain = lodash.last(email.split('@'))
                if (!lodash.includes(localStrg.domainWhitelist.v, emailDomain))
                    throw new WIKI.Error.AuthRegistrationDomainUnauthorized()
            }
            // Check if email already exists
            ...
            if (!usr) {
                ...
                // Assign to group(s)
                if (lodash.get(localStrg, 'autoEnrollGroups.v', []).length > 0)
                ...
                if (verify) {
                    ...
                    await WIKI.mail.send({
                        ...
                        data: {
                            ...
                            content:
                                'Click the button below in order to verify your account and gain access to the wiki.',
                            buttonLink: `${WIKI.config.host}/verify/${verificationToken}`,
                            buttonText: 'Verify'
                        },
                        text:
                            `You must open the following link in your browser to verify your account and gain access to the wiki: ${WIKI.config.host}/verify/${verificationToken}`
                    })
                }
                return true
            } else {
    ...
    static async logout(context) {
        ...
        const provider = lodash.find(WIKI.auth.strategies, ['key', usr.providerKey])
        return provider.logout ? provider.logout(provider.config, context) : '/'
    }

    static async getGuestUser() {
        const user = await WIKI.models.users.query().findById(2).withGraphJoined('groups').modifyGraph(
            'groups',
            (builder) => {
                builder.select('groups.id', 'permissions')
            }
