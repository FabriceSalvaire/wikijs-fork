        ...
        async renameAsset(obj, args, context) {
            try {
                ...
                if (asset) {
                    ...
                    const assetSourcePath = (asset.folderId)
                        ? hierarchy.map((h) => h.slug).join('/') + `/${asset.filename}`
                        : asset.filename
                    ...
                    const assetTargetPath = (asset.folderId)
                        ? hierarchy.map((h) => h.slug).join('/') + `/${filename}`
                        : filename
