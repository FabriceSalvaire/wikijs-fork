...
export default {
    ...
    async init() {
        if (!lodash.get(WIKI.config, 'letsencrypt.payload', false))
            ...
            WIKI.logger.info(
                `(LETSENCRYPT) Using existing certificate for ${WIKI.config.ssl.domain}, expires on ${WIKI.config.letsencrypt.payload.expires}: [ OK ]`
            )
        }
            ...
            const certResp = await this.acme.certificates.create({
                ...
                challenges: {
                    'http-01': {
                        ...
                        set(data) {
                            WIKI.logger.info(
                                `(LETSENCRYPT) Setting HTTP challenge for ${data.challenge.hostname}: [ READY ]`
                            )
