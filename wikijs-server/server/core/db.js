...
export default {
    ...
    async subscribeToNotifications() {
        const useHA = WIKI.config.ha === true ||
            (typeof WIKI.config.ha === 'string' && WIKI.config.ha.toLowerCase() === 'true') || WIKI.config.ha === 1 ||
            WIKI.config.ha === '1'
        if (!useHA)
            return
        else if (WIKI.config.db.type !== 'postgres') {
            WIKI.logger.warn(
                `Database engine doesn't support pub/sub. Will not handle concurrent instances: [ DISABLED ]`
            )
            return
        }
