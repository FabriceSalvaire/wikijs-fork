            ...
            for (const entity of opts.entities) {
                switch (entity) {
                    ...
                    case 'assets': {
                        ...
                        await pipeline(
                            WIKI.models.knex.select('filename', 'folderId', 'data').from('assets').join(
                                'assetData',
                                'assets.id',
                                '=',
                                'assetData.id'
                    ...
                    case 'comments': {
                        ...
                        const fetchCommentsBatch = async (offset) => {
                            const comments = await WIKI.models.comments.query().offset(offset).limit(50)
                                .withGraphJoined({
                                    author: true,
                                    page: true
                    ...
                    case 'history': {
                        ...
                        const fetchPagesBatch = async (offset) => {
                            const pages = await WIKI.models.pageHistory.query().offset(offset).limit(10)
                                .withGraphJoined({
                                    author: true,
                                    page: true,
                                    tags: true
