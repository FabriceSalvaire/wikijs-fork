...
router.get(['/e', '/e/*splat'], async (req, res, next) => {
    ...
    if (page) {
        ...
        if (req.query.from && tmplCreateRegex.test(req.query.from)) {
            ...
            if (tmplVersionId > 0) {
                // -> From Page Version
                const pageVersion = await WIKI.models.pageHistory.getVersion({
                    pageId: tmplPageId,
                ...
                if (
                    !WIKI.auth.checkAccess(req.user, ['read:history'], {
                ...
                if (
                    !WIKI.auth.checkAccess(req.user, ['read:source'], {
                        path: pageOriginal.path,
...
router.get(['/i', '/i/:id'], async (req, res, next) => {
    ...
    const page = await WIKI.models.pages.query().column(['path', 'localeCode', 'isPrivate', 'privateNS']).findById(
        pageId
    )
    if (!page) {
        lodash.set(res.locals, 'pageMeta.title', 'Page Not Found')
    ...
    if (
        !WIKI.auth.checkAccess(req.user, ['read:pages'], {
