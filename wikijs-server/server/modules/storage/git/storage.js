...
export default {
        ...
        switch (this.config.authType) {
            case 'ssh':
                ...
                if (this.config.sshPrivateKeyMode === 'contents') {
                    try {
                        this.config.sshPrivateKeyPath = path.resolve(
                            WIKI.ROOTPATH,
            ...
            default:
                ...
                if (_.startsWith(this.config.repoUrl, 'http')) {
                    ...
                    originUrl = `https://${encodeURI(this.config.basicUsername)}:${
                        encodeURI(this.config.basicPassword)
                    }@${this.config.repoUrl}`
                }
    ...
    async processFiles(files, user) {
        for (const item of files) {
            ...
            if (!item.binary && contentType) {
                ...
                if (
                    fileExists && !item.importAll &&
                    ((item.before === item.after) || (item.deletions === 0 && item.insertions === 0))
                ) {
                    ...
                    if (assetToRename) {
                        ...
                        WIKI.logger.info(
                            `(STORAGE/GIT) Asset was not found in the DB, nothing to rename: ${item.relPath}`
                        )
                    }
                    continue
                } else if (
                    !fileExists && !item.importAll &&
                    ((item.before > 0 && item.after === 0) || (item.deletions > 0 && item.insertions === 0))
                ) {
                    ...
                    if (assetToDelete) {
                        ...
                        WIKI.logger.info(
                            `(STORAGE/GIT) Asset was not found in the DB, nothing to delete: ${item.relPath}`
                        )
                    }
                    continue
                }
    ...
    async renamed(page) {
        WIKI.logger.info(
            `(STORAGE/GIT) Committing file move from [${page.localeCode}] ${page.path} to [${page.destinationLocaleCode}] ${page.destinationPath}...`
        )
        ...
        await this.git.commit(`docs: rename ${page.path} to ${page.destinationPath}`, [
            sourceFilePath,
            destinationFilePath
        ], {
    ...
    async assetRenamed(asset) {
        ...
        await this.git.commit(`docs: rename ${asset.path} to ${asset.destinationPath}`, [
            asset.path,
    ...
    async syncUntracked() {
            ...
            new stream.Transform({
                ...
                transform: async (page, enc, cb) => {
                    ...
                    if (
                        ...
                        (WIKI.config.lang.namespacing && WIKI.config.lang.code !== page.localeCode)
                    ) {
                        fileName = `${page.localeCode}/${fileName}`
                    }
        ...
        await pipeline(
            WIKI.models.knex.column('filename', 'folderId', 'data').select().from('assets').join(
                'assetData',
                'assets.id',
                '=',
                'assetData.id'
