...
export default {
    ...
    async renamed(page) {
        WIKI.logger.info(
            `(STORAGE/DISK) Renaming file [${page.localeCode}] ${page.path} to [${page.destinationLocaleCode}] ${page.destinationPath}...`
        )
        ...
        await fs.move(path.join(this.config.path, sourceFilePath), path.join(this.config.path, destinationFilePath), {
            overwrite: true
        })
    },
    /**
    ...
    async assetRenamed(asset) {
        ...
        await fs.move(path.join(this.config.path, asset.path), path.join(this.config.path, asset.destinationPath), {
            overwrite: true
        })
    },
